...
`position` 填 `segment id`。插件会用这个序号映射回程序切好的原文分片，再插入图片。找不到对应分片或渲染定位失败时，图片会回退追加到该消息末尾，避免生成结果丢失。若正文整体包在 `<content>...</content>` 中，非最后分片插图会在图片上方补 `</content>`、下方补 `<content>`；最后一个分片会优先把图片放到 `</content>` 外。
