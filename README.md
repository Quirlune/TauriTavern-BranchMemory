...
图片缓存存储在全局 Extension Store 的 `image-v1` 表中，缓存键包含当前聊天 scope、用户楼层号、该楼层的累计消息链指纹和图片配方 hash，保留 70 天；启动、切换聊天或回渲染缓存时会清理过期图片。因此在第 100 楼进入第 70 楼分支时，分叉点以前且尚未过期的楼层图片仍然能复用；分叉后链指纹变化的楼层会按新分支重新生成。修改图片提示词、角色外貌提示词、正则、规划模型或 BizyAir 模板会形成新配方，旧配方图片在过期前仍保留在缓存表中，但不会作为当前配方结果回渲染。开启“把图片缓存为 data URL”时，插件会尽量把 BizyAir 返回的图片下载为 data URL；如果跨域或网络阻止下载，会保留远程 URL 作为回退。
